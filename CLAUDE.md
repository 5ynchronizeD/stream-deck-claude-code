...
- **Could not fully verify Windows GUI focus-activation from an agent's Bash tool.** Low-level Win32 mechanics (`EnumWindows` + PID matching) were confirmed correct against `explorer.exe`'s PID directly. But end-to-end `AppActivate` verification (spawn a real window, steal focus, call `focusSessionWindows`, check `GetForegroundWindow` after) failed in that environment — its own process ancestry runs through `WmiPrvSE.exe`, meaning it has no interactive desktop/window-station access at all (GUI processes like `notepad` didn't even start). This is a sandbox limitation of that specific execution context, not a confirmed bug — but it also means this path is UNVERIFIED end-to-end on a real interactive desktop. Test manually before trusting it.
